...
namespace CaptureCore
{
    public static class CaptureHelper
    {
        ...
        public static IntPtr GetHandle(GraphicsCaptureItem item)
        {
            var processesWithWindows = from p in Process.GetProcesses()
                                       where !string.IsNullOrWhiteSpace(p.MainWindowTitle) && p.MainWindowTitle.Equals(item.DisplayName)
                                       select p;
            return processesWithWindows.FirstOrDefault()?.Handle ?? IntPtr.Zero;
        }
